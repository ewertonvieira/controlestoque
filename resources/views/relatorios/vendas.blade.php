<x-app-layout>
    <x-slot name="header">
        <div class="flex justify-between items-center w-full">
            ...
            <div>
                <form action="{{ route('relatorios.vendas.export', 'csv') }}" method="GET" style="display: inline;">
                    <button type="submit" class="bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded">Exportar CSV</button>
                </form>
                <form action="{{ route('relatorios.vendas.export', 'xlsx') }}" method="GET" style="display: inline;">
                            ...
                            @foreach ($vendas as $venda)
                                @foreach ($venda->itens as $item)
                                    <tr>
                                        ...
                                        <td class="border px-4 py-2">{{ number_format($venda->total, 2, ',', '.') }}</td>
