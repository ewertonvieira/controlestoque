<x-app-layout>
                    ...
                    <table class="table-auto w-full mb-10">
                        <thead>
                            <tr>
                                ...
                                <th class="px-4 py-2 w-60">Ações</th>
                            </tr>
                        </thead>
                        <tbody>
                            @foreach ($categorias as $categoria)
                                <tr>
                                    ...
                                    <td class="border px-4 py-2 truncate max-w-[150px]">{{ $categoria->nome }}</td>
                                    <td class="border px-4 py-2 text-center">
                                        <a href="#" class="text-blue-500 hover:text-blue-700" onclick="showModal({{ $categoria }})">Ver</a>
                                        <a href="#" class="text-yellow-500 hover:text-yellow-700" onclick="showEditModal({{ $categoria }})">Editar</a>
    ...
    <div id="create-modal" class="hidden fixed inset-0 bg-gray-600 bg-opacity-50 overflow-y-auto h-full w-full">
        <div class="relative top-20 mx-auto p-5 border w-full max-w-4xl shadow-lg rounded-md bg-white">
            <div class="mt-3 text-left">
                <h3 class="text-lg leading-6 font-medium text-gray-900">Adicionar Nova Categoria</h3>
    ...
    <script>
        function showModal(categoria) {
            document.getElementById('modal-id').innerText = categoria.id;
            document.getElementById('modal-nome').innerText = categoria.nome;
            document.getElementById('product-modal').classList.remove('hidden');
        }

        function showEditModal(categoria) {
            document.getElementById('edit-nome').value = categoria.nome;
            document.getElementById('edit-form').action = '/categorias/' + categoria.id;
