<x-action-section>
    ...
    <x-slot name="content">
        ...
        <div class="mt-3 max-w-xl text-sm text-gray-600">
            <p>
                {{ __('Quando a autenticação de dois fatores está habilitada, você será solicitado a fornecer um token seguro e aleatório durante a autenticação. Você pode obter esse token no aplicativo Google Authenticator do seu telefone.') }}
            </p>
        </div>
            ...
            @if ($showingQrCode)
                <div class="mt-4 max-w-xl text-sm text-gray-600">
                    <p class="font-semibold">
                        @if ($showingConfirmation)
                            {{ __('Para concluir a habilitação da autenticação de dois fatores, escaneie o código QR abaixo usando o aplicativo autenticador do seu telefone ou insira a chave de configuração e forneça o código OTP gerado.') }}
                        @else
                            {{ __('A autenticação de dois fatores está agora habilitada. Escaneie o código QR abaixo usando o aplicativo autenticador do seu telefone ou insira a chave de configuração.') }}
                        @endif
                    </p>
                </div>
